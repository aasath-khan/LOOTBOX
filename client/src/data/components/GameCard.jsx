...
const GameCard = ({ game }) => {
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      setImageLoaded(true);
      console.error('Image load failed:', game.background_image);
    };
  ...
  const toggleLike = (e) => {
    ...
    setLiked((prev) => {
      const updated = !prev;
      localStorage.setItem(`liked-${game.id}`, JSON.stringify(updated));
      return updated;
    });
  };

  const accentColor = '#2d3748'; // Can be adjusted via theme
    ...
    >
      ...
      <div className="p-4 bg-gray-800">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-lg font-medium text-gray-100 truncate">
            {game.name}
          </h2>
          <button
            onClick={toggleLike}
